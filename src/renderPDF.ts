...
function renderInstitutionDate(
  doc: PDFKit.PDFDocument,
  institution: string,
  from: string,
  to: string,
...
function renderSubchild(doc: PDFKit.PDFDocument, child: Node) {
  switch (child.type) {
    ...
    default: {
      throw new TypeError(
        `${child.loc.start.line}:${child.loc.start.column}, Cannot render ${child.type}`,
      );
    }
  }
}
...
function line(
  ...
  doc.save().translate(doc.x, doc.y).scale(0.4).path(icon(iconName)).fill(ACCENT_COLOR).restore();
...
function renderLeftColumn(cv: CV, doc: PDFKit.PDFDocument, width: number) {
  ...
  line(
    ...
    'birth',
    '270-cake',
    `${formatter.format(cv.dateOfBirth)} ${translate(lang, 'in')} ${translate(
      lang,
      cv.placeOfBirth,
    )}`,
  );
  ...
  if (cv.phone)
    line(
      ...
      '166-mobile-phone',
      labelPhone(cv.phone),
      `tel:${cv.phone.replace(/[^\d+]/g, '')}`,
    );
  if (cv.email) line(doc, lang, width, 'email', '11-envelope', cv.email, `mailto:${cv.email}`);
  if (cv.github)
  ...
  if (cv.linkedin)
    line(
      ...
      `https://www.linkedin.com/in/${cv.linkedin}`,
    );
  ...
  for (const [language, skill] of Object.entries(cv.languageSkills)) {
    ...
    for (s = 0; s < Math.floor(skill / 2); s += 1) {
      doc
        .ellipse(doc.x + s * 10 + 4, doc.y + 7, 3)
        .fillColor(DEFAULT_COLOR)
        .fill();
    }
